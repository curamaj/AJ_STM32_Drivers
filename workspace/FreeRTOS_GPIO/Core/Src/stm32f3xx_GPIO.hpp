...
typedef struct{ // This structure is analogous to `GPIO_InitTypeDef` - `stm32f3xx_hal_gpio.h`. This is my HAL layer for GPIO initialization.
    ...
    int pin;                 // pin to configure (0-16.)
    uint16_t bsrr;           // bsrr offset to use to set a pin HI/LO. Will contain a MASK to be used to set value of GPIO_port->BSRR.
    uint32_t mode;           // operation mode - input/output/alternate/analog modes
    uint16_t output;         // output type register - push-pull or pull-up
    uint32_t pupd;           // pull-up/pull-down/push-pull/open-drain
    uint32_t speed;          // operating speed
    uint32_t alternate;      // alternate mode select
}LabGPIO_InitStruct;
...
class LabGPIO_X
{
    ...
    public:
      ...
      void setAsAlternate(uint8_t AF); // [2-13-22 Add On.]
